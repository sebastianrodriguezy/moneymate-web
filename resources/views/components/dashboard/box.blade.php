<div class="bg-gray-50 rounded-md p-4 dark:bg-gray-800" {{ $attributes }}>
  <div class="flex flex-row items-center justify-between">
    <div>
      <div class="flex flex-row items-center gap-3">
        <h3 class="text-md font-medium text-gray-700 dark:text-gray-300">{{ $title }}</h3>
        <template x-if="isLoadingCalc">
          <div role="status">
    ...
    <button class="border-none bg-none cursor-pointer p-1">
      <ion-icon name="refresh" class="text-gray-700 dark:text-gray-300 h-5 w-5" x-on:click="getData"></ion-icon>
    </button>
  </div>
  {{ $slot }}
</div>
