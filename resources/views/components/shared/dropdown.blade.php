<div class="flex flex-col justify-center w-full mb-3">
  ...
  <div
    x-data="dropdown({{ $catalog }}, {{ $onSelect }})"
    ...
    x-id="['dropdown-button-{{ $id }}']"
    class="relative w-full"
  >

    <button
      x-ref="button"
      x-on:click="isLoadingDropdown ? null : toggleDropdown()"
      :aria-expanded="openDropdown"
      :aria-controls="$id('dropdown-button-{{ $id }}')"
      type="button"
    ...
    >
      <template x-for="option in optionsDropdown" :key="option.id">
        <li 
          @click="onClickOptionDropdown(option)"
