<x-layouts.dashboard :user="$user">
  ...
  <div class="w-full flex flex-col bg-gray-50 dark:bg-gray-800 p-4 px-6 rounded-md mb-5 text-gray-700 dark:text-gray-300">
  ...
  <x-shared.table 
    :headings="$tableCols" 
    :endpoint="'/categories'"
    :title="__('messages.categoriesTableTitle')"
    :description="__('messages.categoriesTableSubtitle')"
      ...
      <template x-for="category in rows" :key="category.id">
        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
          ...
          <td class="px-6 py-4">
            <div class="flex flex-row items-center">
              <div 
                ...
                :class="transformColorToClass(category.color)"
              ></div>
              <span 
                class="text-gray-600 dark:text-gray-300 uppercase text-xs"
                x-text="JSON.parse(category.color).name"
              ></span>
            </div>
          </td>
          <td class="px-6 py-4">
          ...
          <td class="px-6 py-4 flex items-center justify-center">
            <button 
              @click="openDrawer('detail', category, '{{ __('messages.categoriesDetailTitle') }}')" 
              type="button" 
